...
export default function PINSettingsScreen() {
  ...
  useState(() => {
    HANDYMAN_PIN.isEnabled().then(setPinEnabled)
  })
...
async function handleVerifyOld(entered) {
  const ok = await HANDYMAN_PIN.verify(entered)
  if (!ok) { setError('Wrong PIN. Try again.'); return }
  setError('')
  if (pinEnabled) {
    await HANDYMAN_PIN.disable()
    setPinEnabled(false)
    setStep(STEP.MENU)
  ...
  await HANDYMAN_PIN.set(entered)
  setPinEnabled(true)
  setNewPIN(''); setError('')
  setStep(STEP.MENU)
 ...
 async function handleEnableConf(entered) {
  ...
  setNewPIN(''); setError('')
  setStep(STEP.MENU)
  Alert.alert('PIN Enabled', 'App will now lock after sign in.')
}
